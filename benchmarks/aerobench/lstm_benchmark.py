...
def main():
    num_trials = 20
    results_df = pd.DataFrame(columns = ['trial_num', 'train_rmse', 'test_rmse'])
    for i in range(num_trials):
        train_rmse, test_rmse = run_trial()
        results_df = pd.concat([pd.DataFrame([[i+1, train_rmse, test_rmse]], columns=results_df.columns), results_df], ignore_index=True)
    
    results_df.to_csv("lstm_benchmark_results.csv")
